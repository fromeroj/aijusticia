...
function IzelRail({ casoNombre }: { casoNombre?: string | null }) {
  const t = useT();
  const [msgs, setMsgs] = useState<{ rol: "user" | "izel"; texto: string; citas?: string[] }[]>([]);
  ...
  const enviar = async () => {
    ...
    try {
      const token = sessionStorage.getItem("aij_tokens");
      const parsed = token ? JSON.parse(token) : null;
      const r = await fetch("/bufetes/query", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(parsed?.access ? { Authorization: `Bearer ${parsed.access}` } : {}),
        },
      ...
      if (r.ok) {
        ...
        setMsgs((m) => [...m, { rol: "izel", texto: d.respuesta || "...", citas }]);
      } else {
      ...
      <div className="border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">{t("izel.consulta")}</p>
          </div>
          {casoNombre && (
            <span className="ml-auto max-w-[80px] truncate rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-medium text-primary">
              {casoNombre}
            </span>
          )}
        </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
        {msgs.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
            <span className="flex size-10 items-center justify-center rounded-full bg-gradient-to-br from-primary to-teal-500 text-sm font-bold text-white">I</span>
            <p className="text-[13px] font-semibold text-foreground">{t("izel.hola")}</p>
            <p className="text-[11px] leading-relaxed text-muted-foreground">{t("izel.descripcion")}</p>
          </div>
        )}
        {msgs.map((m, i) => (
          <div key={i} className={cn("rounded-xl px-3 py-2 text-[12.5px] leading-relaxed",
            m.rol === "user" ? "ml-6 bg-primary/10 text-foreground" : "mr-2 bg-muted text-foreground")}>
        ...
        {escribiendo && (
          <div className="flex items-center gap-1.5 rounded-xl bg-muted px-3 py-2 text-[12px] text-muted-foreground">
